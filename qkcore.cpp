...
//    m_protocol.timeout = 100;
    ...
    {
        ack = m_protocol->sendPacket(&p, true);
    }
...
QkAck QkCore::search()
{
    ...
    QkPacket::Builder::build(&p, pd);
    return m_protocol->sendPacket(&p, true);
}
...
QkAck QkCore::start(int address)
{
    ...
    QkPacket::Builder::build(&p, pd);
    QkAck ack = m_protocol->sendPacket(&p, true);
...
//QkAck QkCore::m_protocol->sendPacket(QkPacket *packet, bool wait)
//{
...
//    QkPacket::Builder::parse(frame, incomingPacket);
